...
// DEINE STRUKTUR (Alle IDs korrekt)
...
bot.on("message", async (msg) => {
  ...
  try {
    // Einfach alles laden, wie vorhin auch
    ...
    const systemPrompt = `
      Du bist der A&R Assistent. Antworte sachlich und direkt.
      Hier sind deine Daten aus Notion:
      - BIOS: ${JSON.stringify(bios)}
      - IPIs/PUBLISHING: ${JSON.stringify(publishing)}
      - STUDIOS: ${JSON.stringify(studios)}
      - CONFIG: ${JSON.stringify(config)}

      Nutze diese Infos für deine Antwort. Wenn du die Info hast, nenne sie. Wenn nicht, sag es.
    `;
    ...
    await bot.sendMessage(msg.chat.id, "Fehler beim Laden der Daten.");
  }
});
