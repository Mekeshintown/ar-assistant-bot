...
function buildNotionProps(data) {
    ...
    notionFields.forEach(f => { 
        ...
        if (incomingValue !== undefined && incomingValue !== null) {
            ...
            if (typeof val === 'object') val = JSON.stringify(val); // Split-Fix
...
async function generateWordDoc(chatId, pageId) {
    ...
    const doc = new Document({
        sections: [{
            children: [
                new Paragraph({ children: [new TextRun({ text: "Labelcopy", bold: true, size: 36 })] }),
                ...
                new Paragraph({ text: "Splits:", bold: true }),
                new Paragraph({ text: lc.Splits || "" })
            ]
        }]
    });
    ...
    return "Word-Datei gesendet. Session beendet.";
}
...
async function handleChat(chatId, text) {
    ...
    if (session && (textLower === "fertig" || textLower === "session löschen")) {
        ...
        return "Check. Session beendet. Normaler Modus aktiv.";
    }
    ...
    if (session) {
        ...
        const extraction = await openai.chat.completions.create({
            model: "gpt-4o",
            messages: [{ role: "system", content: "Extrahiere Labelcopy Felder als JSON. 'Abmischung' -> Mixed by etc. Keine Emojis." }, { role: "user", content: text }],
    ...
    // 4. --- GOOGLE LOGIK (KALENDER & GMAIL - REPARIERT) ---
    ...
    if (calendarTriggers.some(word => textLower.includes(word)) && text.length > 5) {
        const calendarList = await fetchFullDatabase(DB_CALENDARS);
        try {
            const calendar = google.calendar({ version: "v3", auth: oauth2Client });
            const extraction = await openai.chat.completions.create({
                model: "gpt-4o",
                messages: [{ role: "system", content: `Kalender-Assistent. JSON exportieren. Künstler: ${calendarList.map(c => c.Name).join(", ")}` }, { role: "user", content: text }],
                response_format: { type: "json_object" }
            });
            ...
            if (d.type === "read" || textLower.includes("wie sieht")) {
                ...
                await calendar.events.insert({ calendarId: calId, resource: { summary: d.title, start: { dateTime: d.start_iso, timeZone: "Europe/Berlin" }, end: { dateTime: new Date(new Date(d.start_iso).getTime() + 3600000).toISOString(), timeZone: "Europe/Berlin" } } });
    ...
    // 5. --- WISSENS-CHAT ---
    const [config, publishing, studios, bios, artistInfos] = await Promise.all([
        fetchFullDatabase(DB_CONFIG), fetchFullDatabase(DB_PUBLISHING), fetchFullDatabase(DB_STUDIOS), fetchFullDatabase(DB_BIOS), fetchFullDatabase(DB_ARTIST_INFOS)
    ]);
    let history = chatContext.get(chatId) || [];
    history.push({ role: "user", content: text });
    const systemMsg = { role: "system", content: "Professioneller A&R Assistent. Keine Emojis. Wissen: Publishing: " + JSON.stringify(publishing) + ", Studios: " + JSON.stringify(studios) };
